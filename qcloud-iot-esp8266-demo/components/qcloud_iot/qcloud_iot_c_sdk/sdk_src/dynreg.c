...
static int _parse_devinfo(char *jdoc, DeviceInfo *pDevInfo)
{
    ...
    if (enType < 0) {
        Log_e("invalid encrypt type, decrypt may be faild");
        ret = QCLOUD_ERR_FAILURE;
        goto exit;
    }
    ...
    if (eCERT_TYPE != enType) {
        Log_e("encrypt type should be cert type");
        ret = QCLOUD_ERR_FAILURE;
        goto exit;
    }
...
#else
    if (ePSK_TYPE != enType) {
        Log_e("encrypt type should be psk type");
        ret = QCLOUD_ERR_FAILURE;
        goto exit;
    }
