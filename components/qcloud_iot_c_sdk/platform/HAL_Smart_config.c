...
extern void esp_smartconfig_event_cb(smartconfig_status_t status, void *pdata);
...
{
    int ret = 0;
    ret     = esp_wifi_pre_init();
    ret     = esp_wifi_set_mode(WIFI_MODE_STA);
    if (ret != ESP_OK) {
        Log_e("esp_wifi_set_mode failed: %d", ret);
        return ret;
    }
    ret = esp_wifi_start();
    if (ret) {
    ...
    ret = esp_smartconfig_start(esp_smartconfig_event_cb);
    if (ESP_OK != ret) {
