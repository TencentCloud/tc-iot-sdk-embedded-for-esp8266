...
int IOT_Resource_DownloadYield(void *handle, char *buf, uint32_t buf_len, uint32_t timeout_s)
{
    ...
    if ((QCLOUD_RESOURCE_STATE_START_E != resource_handle->download.state) || (resource_handle->download.result_code != 0)) {
    ...
    if (percent == 100) {
        _qcloud_iot_resource_report_download_progress(resource_handle, percent, resource_handle->download.resource_name);
    ...
    switch (type) {
        ...
        case QCLOUD_IOT_RESOURCE_MD5CHECK_E:
            if ((4 != buf_len) || (0 != ((unsigned long)buf & 0x3))) {
                ...
                Log_i("download MD5 check: origin=%s, now=%s", STRING_PTR_PRINT_SANITY_CHECK(resource_handle->download.md5sum),
                      md5_str);
...
int IOT_Resource_ReportMsg(void *handle, char *msg)
{
    QCLOUD_RESOURCE_CONTEXT_T *resource_handle = (QCLOUD_RESOURCE_CONTEXT_T *)handle;
    POINTER_SANITY_CHECK(handle, QCLOUD_RESOURCE_ERRCODE_INVALID_PARAM_E);

    return qcloud_service_mqtt_post_msg(resource_handle->download.channel, msg, QOS0);
}
