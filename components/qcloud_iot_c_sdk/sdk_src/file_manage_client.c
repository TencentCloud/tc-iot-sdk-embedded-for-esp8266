...
typedef struct {
    ...
    IOT_FILE_StateCode state;             /* state */
    ...
    int   request_id;
    void *mutex;
    List *file_wait_post_list;
    void *usr_context;

    int   err;       /* last error code */
    int   report_rc; /* result of _file_manage_report_upgrade_result in IOT_FileManage_FetchYield*/
    Timer report_timer;

    OnFileManageEventUsrCallback usr_cb;
} FileManageHandle;
...
/* static function*/
static int _gen_file_manage_ver_info(char *buf, size_t bufLen, uint16_t res_num, fileInfo *res_list[])
{
...
static int _gen_file_manage_report_msg(char *buf, size_t bufLen, const char *file_name, const char *version,
                                       const char *type, int progress, IOT_FILE_ReportType reportType)
{
    IOT_FUNC_ENTRY;
    ...
    switch (reportType) {
        ...
        case IOT_FILE_TYPE_DOWNLOAD_BEGIN:
        ...
        case IOT_FILE_TYPE_DOWNLOADING:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"report_progress\", \"report\": {\"progress\": "
        ...
        case IOT_FILE_TYPE_SPACE_NOT_ENOUGH:
            ret = HAL_Snprintf(buf, bufLen,
        ...
        case IOT_FILE_TYPE_MD5_NOT_MATCH:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"report_result\", \"report\": {\"progress\": "
                               "{\"resource_name\":\"%s\",\"state\":\"done\", \"result_code\":\"%d\", "
        ...
        case IOT_FILE_TYPE_DOWNLOAD_TIMEOUT:
        case IOT_FILE_TYPE_FILE_NOT_EXIST:
        case IOT_FILE_TYPE_AUTH_FAIL:
        case IOT_FILE_TYPE_UPGRADE_FAIL:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"report_result\", \"report\": {\"progress\": "
                               "{\"resource_name\":\"%s\",\"state\":\"done\", \"result_code\":\"%d\", "
        ...
        case IOT_FILE_TYPE_UPGRADE_BEGIN:
            ret = HAL_Snprintf(buf, bufLen,
        ...
        case IOT_FILE_TYPE_UPGRADE_SUCCESS:
            ret = HAL_Snprintf(buf, bufLen,
        ...
        case IOT_FILE_TYPE_FILE_DEL_SUCCESS:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"del_result\", "
                               "\"report\":{\"progress\":{\"resource_name\":\"%s\",\"state\":\"done\", "
                               "\"result_code\":\"0\", \"result_msg\":\"success\"}, \"version\":\"%s\"}}",
        ...
        case IOT_FILE_TYPE_FILE_DEL_FAIL:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"del_result\", "
                               "\"report\":{\"progress\":{\"resource_name\":\"%s\",\"state\":\"done\", "
        ...
        default:
            IOT_FUNC_EXIT_RC(IOT_FILE_ERR_FAIL);
            break;
    }
    ...
    if (ret < 0) {
        ...
        IOT_FUNC_EXIT_RC(IOT_FILE_ERR_STR_TOO_LONG);
    }

    IOT_FUNC_EXIT_RC(IOT_FILE_ERR_NONE);
}
...
static void _reset_handle_status(void *handle)
{
    ...
    Log_i("reset file_manage handle state!");

    pHandle->state = IOT_FILE_STATE_INITTED;
    pHandle->err   = 0;
...
static int _file_manage_report_progress(void *handle, int progress, IOT_FILE_ReportType reportType)
{
    int               ret          = QCLOUD_ERR_FAILURE;
    char *            msg_reported = NULL;
    FileManageHandle *pHandle      = (FileManageHandle *)handle;

    if (IOT_FILE_STATE_UNINITTED == pHandle->state) {
    ...
    ret = _gen_file_manage_report_msg(msg_reported, MSG_REPORT_LEN, pHandle->file_name, pHandle->version, NULL,
                                      progress, reportType);
    if (QCLOUD_RET_SUCCESS != ret) {
        Log_e("generate file_manage inform message failed");
        pHandle->err = ret;
...
static int _file_manage_report_upgrade_result(void *handle, const char *version, IOT_FILE_ReportType reportType)
{
    FileManageHandle *pHandle = (FileManageHandle *)handle;
    ...
    if (IOT_FILE_STATE_UNINITTED == pHandle->state) {
        Log_e("handle is uninitialized");
        pHandle->err = IOT_FILE_ERR_INVALID_STATE;
        return QCLOUD_ERR_FAILURE;
    }
    ...
    if (!version) {
        ...
        pHandle->err = IOT_FILE_ERR_INVALID_PARAM;
        return QCLOUD_ERR_INVAL;
    }

    len = strlen(version);
    if ((len < FILE_MANAGE_VERSION_STR_LEN_MIN) || (len > FILE_MANAGE_VERSION_STR_LEN_MAX)) {
    ...
    ret = _gen_file_manage_report_msg(msg_upgrade, MSG_REPORT_LEN, pHandle->file_name, version, NULL, 1, reportType);
    if (ret != 0) {
        Log_e("generate file_manage inform message failed");
        pHandle->err = ret;
        ret          = QCLOUD_ERR_FAILURE;
    ...
    if (0 > ret) {
        ...
        pHandle->err = IOT_FILE_ERR_REPORT_UPGRADE_RESULT;
        ret          = QCLOUD_ERR_FAILURE;
        goto exit;
    }

exit:
    if ((IOT_FILE_TYPE_DOWNLOAD_BEGIN != reportType) && (IOT_FILE_TYPE_DOWNLOADING != reportType) &&
        (IOT_FILE_TYPE_UPGRADE_BEGIN != reportType) && (IOT_FILE_TYPE_NONE != reportType)) {
...
static int _file_manage_report_post_result(void *handle, const char *res_token, IOT_FILE_ReportType reportType)
{
    FileManageHandle *pHandle = (FileManageHandle *)handle;
    int               ret;
    char *            msg_post = NULL;
    ...
    ret = _gen_file_manage_report_msg(msg_post, MSG_REPORT_LEN, res_token, NULL, NULL, 0, reportType);
    if (ret != 0) {
        Log_e("generate file_manage inform message failed");
        pHandle->err = ret;
        ret          = QCLOUD_ERR_FAILURE;
...
exit:
    if ((IOT_FILE_TYPE_DOWNLOAD_BEGIN != reportType) && (IOT_FILE_TYPE_DOWNLOADING != reportType) &&
        (IOT_FILE_TYPE_UPGRADE_BEGIN != reportType) && (IOT_FILE_TYPE_NONE != reportType) &&
        (IOT_FILE_TYPE_POST_SUCCESS != reportType) && (IOT_FILE_TYPE_POST_FAIL != reportType)) {
        _reset_handle_status(pHandle);
    }
    HAL_Free(msg_post);
    return ret;
...
static int _add_resouce_info_to_post_list(void *handle, FilePostInfo *info)
{
    IOT_FUNC_ENTRY;
    FileManageHandle *pHandle = (FileManageHandle *)handle;

    HAL_MutexLock(pHandle->mutex);
    if (pHandle->file_wait_post_list->len >= MAX_FILE_WAIT_POST) {
...
static FilePostInfo *_get_file_manage_info_by_request_id(void *handle, int request_id)
{
...
static int _post_file_manage_to_cos(const char *file_manage_url, FilePostInfo *info)
{
...
#define COS_REPLY_TIMEOUT_MS (500)
    ...
    Log_d("post %s(%d) %s to cos", STRING_PTR_PRINT_SANITY_CHECK(info->file_name), info->request_id,
          STRING_PTR_PRINT_SANITY_CHECK(info->file_version));
    void *fp = HAL_FileOpen(info->file_name, "rb");
    if (NULL == fp) {
        Log_e("can not open file %s!", STRING_PTR_PRINT_SANITY_CHECK(info->file_name));
        return QCLOUD_ERR_FAILURE;
    }

    long file_size = HAL_FileSize(fp);
    pUploadHandle  = qcloud_url_upload_init(file_manage_url, file_size, NULL);
    if (!pUploadHandle) {
...
/* callback when file_manage topic msg is received */
static void _file_manage_msg_callback(void *handle, const char *msg, uint32_t msg_len)
{
    FileManageHandle *pHandle     = (FileManageHandle *)handle;
    ...
    if (pHandle->state >= IOT_FILE_STATE_FETCHING) {
    ...
    if (!json_method) {
        Log_e("Get file_manage method failed!");
        goto exit;
    }
    ...
    if (!strcmp(json_method, METHOD_RES_REPORT_VERSION_RSP)) {  // report version resp
        ...
        if (!result_code || strcmp(result_code, "0") != 0) {
            ...
            Log_i("Report file_manage version success!");
            pHandle->usr_cb(pHandle->usr_context, json_str, msg_len, IOT_FILE_EVENT_REPORT_VERSION_RESP);
        }

        HAL_Free(result_code);
        goto exit;
    } else if (strcmp(json_method, METHOD_RES_UPDATE_RESOURCE) == 0) {  // update file_manage
        ...
        if (!pHandle->version || !pHandle->url || !pHandle->file_name || !pHandle->file_type || !md5 || !file_size) {
            Log_e("Get file_manage parameter failed");
            HAL_Free(md5);
            HAL_Free(file_size);
        ...
        pHandle->state = IOT_FILE_STATE_FETCHING;
    } else if (strcmp(json_method, METHOD_RES_DELETE_RESOURCE) == 0) {  // delete file_manage

        pHandle->state = IOT_FILE_STATE_DELETING;
        ...
        if (!pHandle->version || !pHandle->file_type || !pHandle->file_name) {
            ...
            _file_manage_report_upgrade_result(pHandle, pHandle->version, IOT_FILE_TYPE_FILE_DEL_FAIL);
            goto exit;
        }
        rc = pHandle->usr_cb(pHandle->usr_context, pHandle->file_name, strlen(pHandle->file_name),
                             IOT_FILE_EVENT_DEL_FILE);
        if (0 != rc) {
            Log_e("Delete file_manage file(%s) fail", pHandle->file_name);
            _file_manage_report_upgrade_result(pHandle, pHandle->version, IOT_FILE_TYPE_FILE_DEL_FAIL);
            goto exit;
        } else {
            _file_manage_report_upgrade_result(pHandle, pHandle->version, IOT_FILE_TYPE_FILE_DEL_SUCCESS);
        }
        ...
        if (info) {
            ...
            int ret = _post_file_manage_to_cos(res_url, info);
            if (ret == QCLOUD_RET_SUCCESS) {
                _file_manage_report_post_result(pHandle, res_token, IOT_FILE_TYPE_POST_SUCCESS);
            } else {
                _file_manage_report_post_result(pHandle, res_token, IOT_FILE_TYPE_POST_FAIL);
            }
            ...
            pHandle->usr_cb(pHandle->usr_context, json_str, msg_len, IOT_FILE_EVENT_REQUEST_URL_RESP);
            HAL_Free(res_token);
            HAL_Free(res_url);
                          ...
                          OnFileManageEventUsrCallback usr_cb, void *usr_context)
{
    ...
    FileManageHandle *handle;
    handle = HAL_Malloc(sizeof(FileManageHandle));
    if (!handle) {
        Log_e("allocate failed");
        goto exit;
    }

    memset(handle, 0, sizeof(FileManageHandle));
    ...
    rc = qcloud_service_mqtt_event_register(eSERVICE_RESOURCE, _file_manage_msg_callback, handle);
    if (QCLOUD_RET_SUCCESS != rc) {
    ...
    if (handle->file_wait_post_list) {
        ...
        Log_e("create file_wait_post_list fail");
        goto exit;
    }
...
int IOT_FileManage_StartDownload(void *handle, uint32_t offset, uint32_t file_size, uint32_t segment_size)
{
    ...
    if (offset == 0) {
        ret = IOT_FileManage_ResetClientMD5(pHandle);
        if (QCLOUD_RET_SUCCESS != ret) {
            Log_e("initialize md5 failed");
            return QCLOUD_ERR_FAILURE;
        }
...
int IOT_FileManage_IsFetching(void *handle)
{
    ...
    return (IOT_FILE_STATE_FETCHING == pHandle->state);
}
...
int IOT_FileManage_IsFetchFinish(void *handle)
{
    ...
    return (IOT_FILE_STATE_FETCHED == pHandle->state);
}
...
int IOT_FileManage_FetchYield(void *handle, char *buf, uint32_t buf_len, uint32_t timeout_s)
{
    ...
    if (IOT_FILE_STATE_FETCHING != pHandle->state) {
        pHandle->err = IOT_FILE_ERR_INVALID_STATE;
        return QCLOUD_ERR_FAILURE;
    }
    ...
    if (ret < 0) {
        ...
        if (ret == QCLOUD_ERR_HTTP_AUTH) {  // OTA auth failed
            ...
            pHandle->err       = IOT_FILE_ERR_FETCH_AUTH_FAIL;
        ...
        _file_manage_report_progress(pHandle, 0, IOT_FILE_TYPE_DOWNLOAD_BEGIN);
    ...
    if (percent == 100) {
        _file_manage_report_progress(pHandle, percent, IOT_FILE_TYPE_DOWNLOADING);
...
int IOT_FileManage_ReportVersion(void *handle, uint16_t res_num, fileInfo *res_list[])
{
    ...
    buff_len = res_num * FILE_MANAGE_INFO_LEN_MAX + 128;
    if (NULL == (msg_informed = HAL_Malloc(buff_len))) {
        Log_e("allocate for msg_informed failed");
        pHandle->err = IOT_FILE_ERR_MALLOC;
        ret          = QCLOUD_ERR_MALLOC;
        goto do_exit;
    }

    ret = _gen_file_manage_ver_info(msg_informed, buff_len, res_num, res_list);
    if (QCLOUD_RET_SUCCESS != ret) {
...
{
    FileManageHandle *pHandle = (FileManageHandle *)handle;
    return _file_manage_report_upgrade_result(handle, pHandle->version, IOT_FILE_TYPE_UPGRADE_BEGIN);
}
...
int IOT_FileManage_ReportUpgradeSuccess(void *handle, const char *version)
{
    ...
    if (!version) {
        ret = _file_manage_report_upgrade_result(handle, pHandle->version, IOT_FILE_TYPE_UPGRADE_SUCCESS);
    } else {
        ret = _file_manage_report_upgrade_result(handle, version, IOT_FILE_TYPE_UPGRADE_SUCCESS);
    }
...
int IOT_FileManage_ReportUpgradeFail(void *handle, const char *version)
{
    ...
    if (!version) {
        ret = _file_manage_report_upgrade_result(handle, pHandle->version, IOT_FILE_TYPE_UPGRADE_FAIL);
    } else {
        ret = _file_manage_report_upgrade_result(handle, version, IOT_FILE_TYPE_UPGRADE_FAIL);
    }
...
int IOT_FileManage_ReportSpaceNotEnough(void *handle, const char *version)
{
    ...
    if (!version) {
        ret = _file_manage_report_upgrade_result(handle, pHandle->version, IOT_FILE_TYPE_SPACE_NOT_ENOUGH);
    } else {
        ret = _file_manage_report_upgrade_result(handle, version, IOT_FILE_TYPE_SPACE_NOT_ENOUGH);
    }
...
int IOT_FileManage_Ioctl(void *handle, IOT_FILE_CmdType type, void *buf, size_t buf_len)
{
    ...
    switch (type) {
        case IOT_FILE_CMD_FETCHED_SIZE:
        ...
        case IOT_FILE_CMD_FILE_TYPE:
            strncpy(buf, pHandle->file_type, buf_len);
            ((char *)buf)[buf_len - 1] = '\0';
            break;

        case IOT_FILE_CMD_VERSION:
            strncpy(buf, pHandle->version, buf_len);
            ((char *)buf)[buf_len - 1] = '\0';
            break;

        case IOT_FILE_CMD_MD5SUM:
            strncpy(buf, pHandle->md5sum, buf_len);
            ((char *)buf)[buf_len - 1] = '\0';
            break;

        case IOT_FILE_CMD_FILE_NAME:
            strncpy(buf, pHandle->file_name, buf_len);
            ((char *)buf)[buf_len - 1] = '\0';
            break;

        case IOT_FILE_CMD_CHECK_FIRMWARE:
            if ((4 != buf_len) || (0 != ((unsigned long)buf & 0x3))) {
                Log_e("Invalid parameter");
                pHandle->err = IOT_FILE_ERR_INVALID_PARAM;
                return QCLOUD_ERR_FAILURE;
            } else if (pHandle->state != IOT_FILE_STATE_FETCHED) {
                pHandle->err = IOT_FILE_ERR_INVALID_STATE;
                ...
                if (0 == strcmp(pHandle->md5sum, md5_str)) {
                    ...
                    *((uint32_t *)buf) = 0;
                    ...
                    _file_manage_report_upgrade_result(pHandle, pHandle->version, IOT_FILE_TYPE_MD5_NOT_MATCH);
                }
                return 0;
            }
...
int IOT_FileManage_Post_Request(void *handle, uint32_t timeout_ms, const char *file_name, char *file_version,
                                char *file_type)
{
    POINTER_SANITY_CHECK(handle, IOT_OTA_ERR_INVALID_PARAM);
    FileManageHandle *pHandle      = (FileManageHandle *)handle;
    ...
    if (QCLOUD_RET_SUCCESS != ret) {
        Log_e("add file_manage to post list fail,rc:%d", ret);
        goto exit;
    }
    ...
    char *file_name_post = strrchr(file_name, '/');
    if (!file_name_post) {
        file_name_post = (char *)file_name;
    } else {
        file_name_post += 1;
    }
    ret = _gen_file_manage_report_msg(msg_reported, MSG_REPORT_LEN, file_name_post, file_version, file_type,
                                      pHandle->request_id, IOT_FILE_TYPE_REQUEST_URL);
    if (QCLOUD_RET_SUCCESS != ret) {
        Log_e("generate file_manage inform message failed");
        goto exit;
    }
