...
#include "qrcode.h"
      ...
      WIFI_PROV_SIMPLE_CONFIG_ENABLE || WIFI_PROV_BT_COMBO_CONFIG_ENABLE || WIFI_PROV_NO_SENSE_CONFIG_ENABLE)
...
typedef struct _ProductDataDefine {
    ...
    ;  //! TYPE_DEF_TEMPLATE_STRING m_color_temp[32];
...
static void _init_data_template(void)
{
    ...
    // strncpy(sg_ProductData.m_color_temp, "85", MAX_STR_NAME_LEN);
    // sg_ProductData.m_color_temp[strlen("85")] = '\0';
    ...
    sg_DataTemplate[4].data_property.data_buff_len = 32;
    sg_DataTemplate[3].data_property.type          = TYPE_TEMPLATE_INT;  //! TYPE_TEMPLATE_STRING
...
static void OnActionCallback(void *pClient, const char *pClientToken, DeviceAction *pAction)
{
    ...
    // do blink
    Log_i("do action: light blink, period : %d ms", period);
    for (i = 0; i < 6; i++) {
        if (i % 2)
            led_pwm_set_brightness(1, sg_ProductData.m_color, sg_ProductData.m_brightness);
        else
            led_pwm_set_brightness(0, sg_ProductData.m_color, sg_ProductData.m_brightness);
        HAL_SleepMs(period * 1);
    }
...
static void deal_down_stream_user_logic(void *client, ProductDataDefine *light)
{
    ...
    /** hardware control **/
    Log_i("--> hardware control [switch:%d color : %d, brightness : %d]", light->m_light_switch, light->m_color,
          light->m_brightness);
    led_pwm_set_brightness(light->m_light_switch, light->m_color, light->m_brightness);
...
static void button_event_handle(void *client)
{
    if (get_button_state(GPIO_BUTTON_1) == 0) {
        sg_status = 0;
        memset(sg_message, 0, MAX_EVENT_STR_MESSAGE_LEN);
        strcpy(sg_message, "button pressed.");
        IOT_Event_setFlag(client, FLAG_EVENT0);
    }
}
...
static void cycle_report(Timer *reportTimer)
{
    ...
    if (expired(reportTimer)) {
        ...
        countdown_ms(reportTimer, 60000);  // report every mins
    }
}
...
int gen_wifi_config_qrcode(char *page)
{
    int        ret = 0;
    DeviceInfo devInfo;
    char       qcloud_payload[256] = {0};
    ret                            = HAL_GetDevInfo((void *)&devInfo);
    HAL_Snprintf(qcloud_payload, 256, "https://iot.cloud.tencent.com/iotexplorer/device?page=%s&productId=%s", page,
                 devInfo.product_id);
    Log_i("Scan this QR code from the Wechat for Provisioning.");
    qrcode_display(qcloud_payload);
    return ret;
}
...
static bool qcloud_wifi_config_proc()
{
    ...
    Timer timer;
    int   count = 0;
    gen_wifi_config_qrcode("softap");
#if WIFI_PROV_SOFT_AP_ENABLE
    WiFiConfigParams apConf = {"lianlian", NULL, 6};
    ...
    while ((rc == QCLOUD_RET_SUCCESS) && (false == wifi_config_result_success) && !expired(&timer)) {
        Log_d("wait wifi config result... %d", count++);
        set_wifi_led_state(count % 2);
        HAL_SleepMs(1000);
    }
    qiot_wifi_config_stop();
...
#if WIFI_PROV_NO_SENSE_CONFIG_ENABLE
    WiFiConfigParams staConf = {"L-004", "iot2021$", 6};
    rc                       = qiot_wifi_config_start(WIFI_CONFIG_TYPE_NO_SENSE, &staConf, _wifi_config_result_cb);
    countdown(&timer, 500);
    while ((rc == QCLOUD_RET_SUCCESS) && (false == wifi_config_result_success) && !expired(&timer)) {
        Log_d("wait wifi config result... %d", count++);
        set_wifi_led_state(count % 2);
...
#endif  // WIFI_PROV_NO_SENSE_CONFIG_ENABLE
